...
var  halfUlBox = (ly_friend_ul.children[0].offsetWidth + 16);

var ly_best_wangguo = ly_friend_ul.children[4];
var ly_clone_wangguo = ly_best_wangguo.cloneNode(true);
ly_friend_ul.insertBefore(ly_clone_wangguo, ly_friend_ul.children[0]);

var ly_best_sichuan = ly_friend_ul.children[6];
var ly_clone_sichuan = ly_best_sichuan.cloneNode(true);
ly_friend_ul.insertBefore(ly_clone_sichuan, ly_friend_ul.children[1]);

ly_friend_ul.style.width = ly_friend_ul.children.length * halfUlBox + "px";
ly_friend_ul.style.left = -halfUlBox * 2 + "px";

// 定义索引值
var li_num;
var num2 = halfUlBox * 2;
for (var i = 0;i < li_best_lis.length;i++) {
    ...
    li_best_lis[i].onclick = function () {
        ...
        // animate(ly_friend_ul , -li_num * ly_friend_company.offsetWidth/2);
...
ly_best_right.onclick = function () {
    if(flag) {
        ...
        ly_index++;
        if (ly_index === 2) {
            ly_friend_ul.style= "transition:none; left: 0;";
            ly_index = 0;
        }
        if (ly_index === 1) {
            // 当点击右箭头的时候，绑定点击li的事件
            // li_best_lis[ly_index].onclick();
            ly_friend_ul.style.left = -(2 * num2) +'px';
        } else {
            setTimeout(function() {
                ly_friend_ul.style.transition = "left .3s ease-in";
                ly_friend_ul.style.left = -num2 +'px';
            }, 20)
            // 当移动到最后一张图片的时候，让它继续移动到克隆的盒子
            /*ly_friend_ul.style.transition = "left .3s"
            ly_friend_ul.style.left = -(ly_index + 1) * num2 +'px';
            // 取消li的蓝色背景，让第一个li背景变蓝
            for (var i = 0;i < li_best_lis.length;i++) {
                li_best_lis[i].style.background = '';
            }
...
var ly_best_sichuan = ly_friend_ul.children[5];
var ly_clone_sichuan = ly_best_sichuan.cloneNode(true);
ly_friend_ul.insertBefore(ly_clone_sichuan, ly_friend_ul.children[0]);
*/
